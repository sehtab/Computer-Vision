# -*- coding: utf-8 -*-
"""
Created on Mon Nov 30 00:20:26 2020
...
def segment(frame, threshold_min=25):
    ...
    if len(contours)== 0:
        ...
        hand_segment = max(contours, keys = cv2.contourArea)
        return (thresholded, hand_segment)
    
def count_fingers(thresholded, hand_segment):
    conv_hull = cv2.convexHull(hand_segment)
    #top
    top = tuple(conv_hull[:, :, 1].argmin()[0])
    bottom = tuple(conv_hull[:, :, 1].argmin()[0])
    left = tuple(conv_hull[:, :, 0].argmin()[0])
    right = tuple(conv_hull[:, :, 0].argmin()[0])
...
while True:
    ...
    if num_frames < 60:
        calc_accum_avg(frame, accumulated_weight)
